...
public class AdminController {
    ...
    @ResponseBody
    public ResponseEntity<?> actualizarUsuario(@PathVariable Integer id, @RequestBody Map<String, Object> usuarioActualizadoMap) {

        System.out.println("🔧 API: Actualizando usuario ID: " + id);
        try {
            Usuario usuario = usuarioService.findById(id).orElseThrow(() -> new RuntimeException("Usuario no encontrado con ID: " + id));
            ...
            if (rolData instanceof Map) {
                @SuppressWarnings("unchecked") Map<String, Object> rolMap = (Map<String, Object>) rolData;
    ...
    public String dashboard(Model model, HttpSession session) {
        ...
        List<Usuario> usuariosRecientes = usuarioService.buscarConFiltrosOr(null, null, null, null).stream().limit(5).toList();
    ...
    @GetMapping("/usuarios")
    public String listarUsuarios(@RequestParam(required = false) Integer id, @RequestParam(required = false) String nombre, @RequestParam(required = false) String email, @RequestParam(required = false) String estado, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, @RequestParam(defaultValue = "id") String sortBy, @RequestParam(defaultValue = "asc") String sortDir, Model model, HttpSession session) {
        ...
        Page<Usuario> paginaUsuarios = usuarioService.buscarUsuariosPaginados(idBusqueda, nombreBusqueda, emailBusqueda, estadoBusqueda, pageable);
    ...
    @GetMapping("/tareas")
    public String listarTareas(@RequestParam(required = false) String titulo, @RequestParam(required = false) String prioridad, @RequestParam(required = false) String estado, @RequestParam(required = false) String usuario, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, @RequestParam(defaultValue = "id") String sortBy, @RequestParam(defaultValue = "desc") String sortDir, Model model, HttpSession session) {
        ...
        Page<Tarea> paginaTareas = tareaService.buscarTareasPaginadas(tituloBusqueda, prioridadBusqueda, estadoBusqueda, usuarioBusqueda, pageable);
    ...
    @ResponseBody
    public Map<String, Object> eliminarPermanente(@PathVariable Integer id, HttpSession session) {
